...
retr <-  retr %>%   mutate( HMi1 =  as.numeric(  map_chr(HmLine, ~ str_split(.,"")[[1]][1])),   
              ...
              mutate(   Visi1 =  as.numeric(  map_chr(VisLine, ~ str_split(.,"")[[1]][1] )),   
               ...
               mutate(HMi9 =ifelse( is.na(HMi9),0,HMi9)) %>% 
  ...
  n_vis_behind <- gamewinProb %>%   filter( ( (winner == "Hm" & net > 0) |  (winner == "Vis" & net < 0) ) ) %>%
                                    filter(inning != "9") %>%  mutate(absscore = abs(net)) %>% 
                                    group_by(inning,absscore)  %>%   summarise( nabs = n(),.groups = "keep") 
    
  
  n_hm_behind <- gamewinProb %>%   filter( (winner == "Vis" & net > 0) |  (winner == "Hm" & net < 0)  ) %>%
                                    filter(inning != "9") %>%
                                   group_by(inning,absscore)  %>%   summarise( nabs = n(),.groups = "keep") 
 ...
 score_counts <-  gamewinProb %>% filter(net < 0) %>% 
  ...
  merged_counts_vs <-  score_counts %>% filter(winner == "Vis") %>% left_join( n_vis_behind) %>%
                        mutate( prob =  n/nabs)  %>%  filter( (net >= -4) & (net <= 0)) %>% 
                       mutate( net = as.factor(net)) %>% filter( prob < .5 & prob > .04)  %>%
                      mutate( money_line = 100* ((1 -prob)/(prob)) )  
 
 
  
  merged_counts_hm <-  score_counts %>% filter(winner == "Hm") %>% left_join( n_hm_behind) %>%
                        mutate( prob =  n/nabs)  %>%  filter( (net >= -4) & (net <= 0)) %>% 
                      mutate( net = as.factor(net)) %>% filter( prob < .5 & prob > .04)  %>%
                       mutate( money_line = 100* ((1 -prob)/(prob)) )  
